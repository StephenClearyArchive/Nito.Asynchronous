...
namespace Server
{
    public partial class FormServerMain : Form
    {
        ...
        /// </summary>
        private ServerTcpSocket ListeningSocket;
        ...
        private class ChildSocketContext
        {
            ...
            /// </summary>
            public SocketPacketProtocol Protocol { get; set; }
        ...
        /// </summary>
        private Dictionary<ServerChildTcpSocket, ChildSocketContext> ChildSockets = new Dictionary<ServerChildTcpSocket, ChildSocketContext>();
        ...
        private void buttonDisconnect_Click(object sender, EventArgs e)
        {
            ...
            foreach (KeyValuePair<ServerChildTcpSocket, ChildSocketContext> childSocket in ChildSockets)
            {
                try
                {
                    childSocket.Key.ShutdownAsync();
                    childSocket.Value.State = ChildSocketState.Disconnecting;
                }
                catch (Exception ex)
                {
                    // Make a note of the error to handle later
            ...
            foreach (KeyValuePair<ServerChildTcpSocket, Exception> error in SocketErrors)
            {
                textBoxLog.AppendText("Child Socket error disconnecting from " + error.Key.RemoteEndPoint.ToString() + ": [" + error.Value.GetType().Name + "] " + error.Value.Message + Environment.NewLine);
                ResetChildSocket(error.Key);
            }
        ...
        private void checkBoxKeepalive_CheckedChanged(object sender, EventArgs e)
        {
            timer.Enabled = checkBoxKeepalive.Checked;
        }
    }
}
